...
Module.register("MMM-Formula1", {
	...
	prepareData(schedule) {
		...
		if (currentround) {
			templateScheduleData = {
				...
				nextRaceName:
					currentround < schedule.length ? schedule[currentround].raceName : null,
				...
				nextRaceDate:
					currentround < schedule.length
						? this.formatDateAndTime(schedule[currentround])
						: null,
				...
				sprintQualifyingDate: this.formatDateAndTime(
					schedule[currentround - 1].SprintQualifying
				),
	...
	findFanConstructorPosition(standings) {
		...
		for (var i = 0; i < standings.length; i++) {
			if (
				standings[i].Constructor.constructorId ===
				this.config.fanConstructorCode.toLowerCase()
			) {
				return standings[i];
			}
		}
